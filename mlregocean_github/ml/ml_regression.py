...
ALL_NAME_MODELS = ['RLR','RFR','Polyfit']#'KRR'
def train_models(path_to_save_models=data_load_SYKE_S3.PATH_TO_MODELS_SYKE,
               bands_try=list(data_load_SYKE_S3.bands_try_SYKE.keys()),
               name_models_try=models.ALL_NAME_MODELS,n_jobs=-1):

    ###1. Read dataset
    skdata_X_train, _, skdata_y_train, _ =  data_load_SYKE_S3.load_SYKE()
    mean_y = np.mean(skdata_y_train)
    skdata_y_train_norm = skdata_y_train - mean_y


    for name_bands in bands_try:
        bands = data_load_SYKE_S3.bands_try_SYKE[name_bands]
